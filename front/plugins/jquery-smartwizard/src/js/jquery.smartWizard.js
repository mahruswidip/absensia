...
(function (factory) {
  ...
  class SmartWizard {
    ...
    _setElements() {
      ...
      if (
        this.options.anchorSettings.enableAllAnchors !== true ||
        this.options.anchorSettings.anchorClickable !== true
      ) {
    ...
    _setEvents() {
      ...
      $(this.steps).on("click", (e) => {
        ...
        if (
          this.options.anchorSettings.enableAnchorOnDoneStep === false &&
          this._isDone(idx)
        ) {
          return true;
        }
    ...
    _createToolbar(position) {
      ...
      var toolbar = $("<div></div>")
        .addClass("toolbar toolbar-" + position)
        .attr("role", "toolbar");
      ...
      let btnNext =
        this.options.toolbarSettings.showNextButton !== false
          ? $("<button></button>")
              ...
              .addClass("btn btn-info sw-btn-next")
              .attr("type", "button")
          : null;
      let btnPrevious =
        this.options.toolbarSettings.showPreviousButton !== false
          ? $("<button></button>")
              ...
              .addClass("btn btn-info sw-btn-prev")
              .attr("type", "button")
          : null;
    ...
    _loadStep(idx) {
      ...
      let getStepContent = this._triggerEvent("stepContent", [
        selStep,
        idx,
        stepDirection,
      ]);
    ...
    _keyNav(e) {
      ...
      if ($.inArray(e.which, this.options.keyboardSettings.keyLeft) > -1) {
        ...
        $.inArray(e.which, this.options.keyboardSettings.keyRight) > -1
      ) {
        // right
        this._showNext();
        e.preventDefault();
