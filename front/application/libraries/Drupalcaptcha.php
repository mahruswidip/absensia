...
class Drupalcaptcha
{
    ...
    */
    ...
    {
        $CI =& get_instance();
        if ($CI->session->userdata('captcha_solution')) {
            $CI->session->unset_userdata('captcha_solution');
        }
    ...
    */
    function image_captcha_captcha()
    {
        ...
        $CI->session->set_userdata('image_captcha_'.$seed, $code);

        //$_SESSION['captcha']['solution'] = $code;
        $CI->session->set_userdata('captcha_solution', $code);
    ...
    function _image_captcha_generate_image($code)
    {
        ...
        $width = $character_spacing * $font_size * $character_quantity; // 250;
        $height = 2 * $font_size;
        ...
        // Distort the image.
        $distortion_amplitude = .85 * $font_size * 1 / 10.0;
    ...
    {
        $noise_quantity = ($width * $height) / 2.0 * (1) / 4.0;
